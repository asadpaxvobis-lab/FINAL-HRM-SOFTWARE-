-- Fix attendance metrics to use company timezone when building scheduled start/end.
...
-- Re-run backfill with company timezone.
WITH resolved AS (
  SELECT
    ad.id,
    ad.attendance_date,
    ...
    ad.is_holiday,
    ad.is_weekly_off,
    COALESCE(c.timezone, 'Asia/Karachi') AS company_tz,
  ...
  JOIN public.companies c ON c.id = ad.company_id
  LEFT JOIN LATERAL (
    SELECT esa.shift_id

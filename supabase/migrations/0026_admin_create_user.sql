-- Create login users from the admin UI without a deployed Edge Function.
-- Mirrors supabase/migrations/0003_seed_admin_user.sql auth insert pattern.
...
-- Allow role assignment during user creation from the client (fallback path).
DROP POLICY IF EXISTS user_roles_modify ON public.user_roles;
CREATE POLICY user_roles_modify ON public.user_roles
  FOR ALL TO authenticated
  USING (
    (public.user_has_permission('user.update') OR public.user_has_permission('user.create'))
    AND EXISTS (
      SELECT 1 FROM public.users u
      WHERE u.id = user_roles.user_id AND u.company_id = public.current_user_company_id()
    )
  )
  WITH CHECK (
    (public.user_has_permission('user.update') OR public.user_has_permission('user.create'))
